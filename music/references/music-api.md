...
Models apply to the generation commands (`generate`, `remix`, `instrumental`, `soundtrack`, `track`, `region-edit`, `extend`). `stem` uses its own separation models (see below).

## Commands

| Command | Kind | Inputs | Constraints | Output |
|---------|------|--------|-------------|--------|
| `generate` | async (task) | `--prompt` and/or `--lyrics`; `--model`, `--style`, `--title`, `--instrumental`, `--vocal-id` | At least one of `--prompt` / `--lyrics`. `--vocal-id` from a prior `vocal-clone` | Task → song (audio URL, title, duration, credits) |
...
| `region-edit` | async (task) | `--audio` XOR `--provider-song-id`; `--lyrics`; `--edit-start` (ms), `--edit-end` (ms); `--model` | `--edit-start >= 12000`; `--edit-end - --edit-start >= 3000` | Task → song with the region rewritten |
...
| `vocal-clone` | sync | `--audio` | — | Reusable Vocal ID (pass to `generate --vocal-id`) |
...
| Audio (`track`, `region-edit`) | mp3, m4a, wav | 10 MB |
| Image (`soundtrack`) | jpg, jpeg, png, webp | 10 MB |
| Video (`soundtrack`) | mp4, mov, avi, mkv, webm | 10 MB |

Time-range flags (`--generate-start`, `--generate-end`, `--edit-start`, `--edit-end`) are in **milliseconds**.

## Async vs sync

- **Async** (`generate`, `remix`, `instrumental`, `soundtrack`, `track`, `region-edit`, `extend`, `cover`): return a task. Poll with `listenhub music get <taskId>` or let the CLI poll internally. Music generation can take up to ~10 minutes.
- **Sync** (`recognize`, `describe`, `stem`, `vocal-clone`): return results directly in the same call.
