export const languages = new Set('zh en ja ko vi th id ms fil hi ar fr de es pt ru it nl sv da fi no el pl cs hu ro bg hr sk'.split(' '));
function window(start, end, samples) {
    if (start === undefined ||
        end === undefined ||
...
export function transcriptEvidence(transcript, sampleFrames) {
...
export function assertEvidenceWav(bytes, sampleFrames) {
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const tag = (at) => new TextDecoder().decode(bytes.subarray(at, at + 4));
    ...
    for (let at = 12; at + 8 <= bytes.length;) {
        ...
        if (tag(at) === 'data')
            validData = length === sampleFrames * 2;
        at = body + length + (length % 2);
    }
    if (!validFormat || !validData)
        throw new Error('Evidence must be 16 kHz mono PCM s16 with the declared sample count');
}
