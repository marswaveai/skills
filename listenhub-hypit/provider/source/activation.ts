  ...
  hostFacets: [
    createRuntimeEndpointAdapterFacet({
      ...
      activate(context) {
        ...
        runtimeConfigExact(
          config,
          ['baseUrl', 'apiKey', 'pollIntervalMs', 'timeoutMs'],
          'ListenHub',
        );
        const apiKey = runtimeConfigCredentialRef(
          config.apiKey,
          'ListenHub API key',
        );
        if (!apiKey || !context.pool)
          throw new Error(
            'ListenHub requires apiKey credential reference and pool',
          );
        return {
          endpoint: createListenHubProvider({
            ...
            apiKey,
            baseUrl:
              runtimeConfigString(config.baseUrl, 'ListenHub baseUrl') ??
              'https://api.listenhub.ai/openapi',
            pollIntervalMs: runtimeConfigPositiveInteger(
              config.pollIntervalMs,
              'pollIntervalMs',
            ),
            timeoutMs: runtimeConfigPositiveInteger(
              config.timeoutMs,
              'timeoutMs',
            ),
          }),
