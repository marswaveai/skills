...
import {
  ...
  type BlobRef,
  type CredentialRef,
  type EndpointInvocationContext,
...
function alignmentRequest(value: unknown): AlignmentRequest {
  ...
  return request as unknown as AlignmentRequest;
}

export function supportsAlignment(request: EndpointRequest) {
  ...
  return languages.has(String(constraints.language))
    ...
    : {
        ...
        reason:
          'ListenHub transcription does not support this language; use one of the 30 documented spoken-language codes',
      };
}

async function resource(context: EndpointInvocationContext, blob: BlobRef) {
...
export function createListenHubProvider(options: Options) {
  ...
  if (
    ...
    throw new Error(
      'baseUrl must be HTTPS (or loopback HTTP for local tests), without credentials/query/fragment',
    );
  const baseUrl = url.href.replace(/\/+$/u, '');
  const fetcher = options.fetch ?? fetch;
  const timeoutMs = options.timeoutMs ?? 600_000;
  const pollIntervalMs = options.pollIntervalMs ?? 3000;
  const secret = (context: EndpointInvocationContext) =>
    text(context.credentials.apiKey?.secret, 'ListenHub API key');
  const call = async (path: string, apiKey: string, body?: unknown) => {
    ...
    try {
      ...
      throw new Error(
        `ListenHub ${path} transport failed; submission outcome may be unknown. Do not automatically resubmit paid work`,
      );
    }
    ...
    if (value.errno !== undefined && value.errno !== 0)
      throw new Error(
        `ListenHub ${path} business error ${String(value.errno)}`,
      );
  ...
  return defineEndpointPackage({
    ...
    async readPricing(context) {
      ...
      const estimate = await call(
        path,
        apiKey,
        isAlignment ? {durationMs} : imageParameters,
      );
      return [
        {
          source: `${baseUrl}${path}`,
    ...
    capabilities: [
      {
        ...
        async handler(context) {
          ...
          const uploaded = await fetcher(uploadUrl, {
            method: 'PUT',
            headers: object(upload.headers) as Record<string, string>,
          ...
          while (Date.now() <= deadline) {
            ...
            if (task.status === 'failed')
              throw new Error(
                `ListenHub transcription ${id} failed; inspect the existing task before retrying`,
              );
            if (task.status === 'completed') {
              ...
              return {
                value: {
                  ...
                  value: canonicalize(
                    transcriptEvidence(
                      transcript as unknown as Transcript,
          ...
          throw new Error(
            `ListenHub transcription ${id} wait timed out; the task may still be running. Inspect it before resubmitting`,
          );
        },
      },
      {
        ...
        supports(request) {
          ...
          return ['image/png', 'image/jpeg'].includes(String(mime))
            ...
            : {
                status: 'unsupported',
                reason:
                  'Background removal accepts PNG/JPEG; convert the source image first',
              };
        },
        async handler(context) {
          const source = object(context.need.constraints).source as BlobRef;
          ...
          while (Date.now() <= deadline) {
            const task = await call(
              `${imagePath}/tasks/${encodeURIComponent(id)}`,
              apiKey,
            );
            if (task.status === 'fail')
              throw new Error(
                `ListenHub image ${id} failed; inspect the existing task before retrying`,
              );
            if (task.status === 'success') {
              const images = task.images as Array<{url?: string}> | undefined;
              const imageUrl = new URL(
                text(images?.[0]?.url, `image URL for ${id}`),
              );
              if (
              ...
              if (!response.ok)
                throw new Error(
                  `Image download for ${id} returned HTTP ${response.status}`,
                );
              const png = keyImage(
          ...
          /* eslint-enable no-await-in-loop */
          throw new Error(
            `ListenHub image ${id} wait timed out; inspect this task before resubmitting`,
          );
        },
      },
    ],
